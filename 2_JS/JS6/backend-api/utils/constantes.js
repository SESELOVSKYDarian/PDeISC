// valores fijos que uso en varios puntos del backend, para no repetir "numeros magicos"

export const LONGITUD_MINIMA_PALABRA = 3;
export const LONGITUD_MAXIMA_PALABRA = 30;

export const LONGITUD_MINIMA_NOMBRE = 2;
export const LONGITUD_MAXIMA_NOMBRE = 40;

export const DIFICULTADES_VALIDAS = ['facil', 'media', 'dificil'];

export const LIMITE_JSON = '2mb';

export const NOMBRE_COOKIE_SESION_ADMIN = 'admin_session';
export const LONGITUD_CODIGO_VERIFICACION = 6;
export const MINUTOS_CODIGO_VERIFICACION = 10;
export const DIAS_SESION_ADMIN = 7;
export const VALOR_MAXIMO_SCORE = 2147483647;
