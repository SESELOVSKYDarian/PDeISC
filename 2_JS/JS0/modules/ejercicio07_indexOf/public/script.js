...
function getCodigo(v) {
  return v?.codigo ?? v?.código ?? "";
}
...
async function loadData() {
  ...
  document.getElementById("title").textContent = `Método ${getMethodName()}`;
  document.title = `Método ${getMethodName()}`;
  state.selectedOriginalIndex = 0;
  renderSelect();
  renderCurrentVariant();
  if (getMethodName() === "secreto") secretTools.classList.remove("d-none"); else secretTools.classList.add("d-none");
}
